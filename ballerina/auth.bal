...
type AuthHandlers record {
    ListenerAuthHandler fileStoreHandler?;
    ListenerAuthHandler ldapHandler?;
    ListenerAuthHandler jwtHandler?;
    ListenerAuthHandler oauthHandler?;
};

isolated AuthHandlers authHandlers = {};

const string FILE_STORE_HANDLER_KEY = "fileStoreHandler";
const string LDAP_STORE_HANDLER_KEY = "ldapStoreHandler";
const string JWT_VALIDATE_HANDLER_KEY = "jwtValidateHandler";
const string OAUTH_VALIDATE_HANDLER_KEY = "oauthValidateHandler";
...
isolated function authenticateWithFileUserStore(http:FileUserStoreConfigWithScopes config, string header)
    ...
    lock {
        if authHandlers.hasKey(FILE_STORE_HANDLER_KEY) {
            handler = <http:ListenerFileUserStoreBasicAuthHandler>authHandlers.get(FILE_STORE_HANDLER_KEY);
        } else {
            handler = new (config.fileUserStoreConfig.cloneReadOnly());
            authHandlers.fileStoreHandler = handler;
        }
    }
...
isolated function authenticateWithLdapUserStoreConfig(http:LdapUserStoreConfigWithScopes config, string header)
    ...
    lock {
        if authHandlers.hasKey(LDAP_STORE_HANDLER_KEY) {
            handler = <http:ListenerLdapUserStoreBasicAuthHandler>authHandlers.get(LDAP_STORE_HANDLER_KEY);
        } else {
            handler = new (config.ldapUserStoreConfig.cloneReadOnly());
            authHandlers.ldapHandler = handler;
        }
    }
    auth:UserDetails|http:Unauthorized authn = handler->authenticate(header);
    string|string[]? scopes = config?.scopes;
...
isolated function authenticateWithJwtValidatorConfig(http:JwtValidatorConfigWithScopes config, 
    ...
    lock {
        if authHandlers.hasKey(JWT_VALIDATE_HANDLER_KEY) {
            handler = <http:ListenerJwtAuthHandler>authHandlers.get(JWT_VALIDATE_HANDLER_KEY);
        } else {
            handler = new (config.jwtValidatorConfig.cloneReadOnly());
            authHandlers.jwtHandler = handler;
        }
    }
...
isolated function authenticateWithOAuth2IntrospectionConfig(
    ...
    lock {
        if authHandlers.hasKey(OAUTH_VALIDATE_HANDLER_KEY) {
            handler = <http:ListenerOAuth2Handler>authHandlers.get(OAUTH_VALIDATE_HANDLER_KEY);
        } else {
            handler = new (config.oauth2IntrospectionConfig.cloneReadOnly());
            authHandlers.oauthHandler = handler;
        }
    }
